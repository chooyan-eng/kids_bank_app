...
- **ニューモーフィック（Neumorphism）デザイン**を採用（`flutter_neumorphic_plus` 使用）
- 背景色: 柔らかいグレー `Color(0xFFE8ECF0)` で統一。カード・ボタンが背景から浮き出るように見える
- カラーアクセント: アンバー（保存・確定）、ライトグリーン（入金）、ライトレッド（出金）で操作を色分け
- フォント: システムデフォルト（日本語対応）。残高表示は太字・大サイズ（42px）
- アイコン: Material Icons を基本。カスタムアイコンは最小限
- ボタン: `NeumorphicButton`（押し込み感のあるアニメーション付き）
- テキストフィールド: `Neumorphic` コンテナ（depth 負値）でインセットの凹み感を演出
- ダイアログ: `Dialog` + `Neumorphic` でシームレスに背景と馴染む
- アニメーション: ニューモーフィックボタンの押し込み効果が自動適用
